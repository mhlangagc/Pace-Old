...
typealias SLPagingViewMoving = ((_ subviews: NSArray)-> ())
typealias SLPagingViewMovingRedefine = ((_ scrollView: UIScrollView, _ subviews: NSArray)-> ())
typealias SLPagingViewDidChanged = ((_ currentPage: NSInteger)-> ())
...
class SLPagingViewSwift: UIViewController, UIScrollViewDelegate {
    ...
    convenience init(items: NSArray, views: NSArray) {
        self.type(of: init)(items: items, views: views, showPageControl:false, navBarBackground:UIColor.whiteColor())
    }
    ...
    */
    init(items: NSArray, views: NSArray, showPageControl: Bool, navBarBackground: UIColor){
        ...
        var i: Int                        = 0
        for item in items{
            if (item as AnyObject).isKind(of: UIView.classForCoder()){
                var v             = item as! UIView
                var vSize: CGSize = v.isKind(of: UILabel.classForCoder()) ? self.getLabelSize(v as UILabel) : v.frame.size
        ...
        if (views.count > 0){
            ...
            for controller in views{
                if (controller as AnyObject).isKind(of: UIView.classForCoder()) {
                    var ctr = controller as! UIView
                    ctr.tag = i
                    controllerKeys.add(NSNumber(value: i))
                }
                else if (controller as AnyObject).isKind(of: UIViewController.classForCoder()){
            ...
            if controllerKeys.count == views.count {
               self.viewControllers = NSDictionary(objects: views as! [Any], forKeys: controllerKeys)
            }
            else{
                var exc = NSException(name: NSExceptionName(rawValue: "View Controllers error"), reason: "Some objects in viewControllers are not kind of UIViewController!", userInfo: nil)
                exc.raise()
            }
        }
    }
    ...
    */
    convenience init(controllers: NSArray, showPageControl: Bool, navBarBackground: UIColor){
        ...
        for ctr in controllers {
            if (ctr as AnyObject).isKind(UIViewController.classForCoder()) {
                views.add(ctr)
                var item  = UILabel()
                item.text = (ctr as AnyObject).title
                items.add(item)
            }
        }
    ...
    */
    convenience init(items: NSArray, controllers: NSArray, showPageControl: Bool, navBarBackground: UIColor){
        ...
        for ctr in controllers {
            if (ctr as AnyObject).isKind(UIViewController.classForCoder()) {
                views.addObject((ctr as AnyObject).value("view")!)
            }
        }
        self.init(items: items, views: views, showPageControl:showPageControl, navBarBackground:navBarBackground)
    }
    ...
    */
    func setCurrentIndex(index: NSInteger, animated: Bool){
        ...
        if(index < 0 || index > self.navigationBarView.subviews.count-1){
            var exc = NSException(name: NSExceptionName(rawValue: "Index out of range"), reason: "The index is out of range of subviews's countsd!", userInfo: nil)
            exc.raise()
        }
    ...
    private func addViews() {
        if self.viewControllers.count > 0 {
            ...
            var i: Int                  = 0
            self.viewControllers.enumerateKeysAndObjects({ key, obj, stop in
    ...
    private func getLabelSize(lbl: UILabel) -> CGSize{
        var txt = lbl.text!
        return txt.size(attributes: [NSFontAttributeName: lbl.font])
    }
    ...
    func tapOnHeader(recognizer: UITapGestureRecognizer){
        if(self.isUserInteraction){
            ...
            var view = self.viewControllers.objectForKey(NSNumber(value: key!)) as! UIView
    ...
    func scrollViewDidScroll(scrollView: UIScrollView) {
        ...
        for obj in self.navItems {
            ...
            var originX  = self.getOriginX(vSize: vSize, idx: CGFloat(i), distance: CGFloat(distance), xOffset: CGFloat(xOffset))
            v.frame      = CGRectMake(originX, 8, vSize.width, vSize.height)
            i += 1
        }
        if (self.pagingViewMovingRedefine != nil) {
            self.pagingViewMovingRedefine!(scrollView, self.navItems)
        }
