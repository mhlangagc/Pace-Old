...
class TrainersAndFriendsLabelCell: BaseCell {
	...
	override func layoutSpecThatFits(_ constrainedSize: ASSizeRange) -> ASLayoutSpec {
		...
		trainersAndFriendsLabelText.style.preferredSize = CGSize(width: constrainedSize.max.width - 130.0, height: 20.0)
		let labelAndButtonStackSpec = ASStackLayoutSpec(direction: .horizontal,
		                                                spacing: 75,
		                                                justifyContent: .start,
		                                                alignItems: .center,
		                                                children: [trainersAndFriendsLabelText, seeAllButton])
